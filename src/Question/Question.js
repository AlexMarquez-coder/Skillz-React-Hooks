...
const Question = (props) => {
    ...
    useEffect(() => {
        ...
        for (let i = 1; i <= rating; i++) {
            setStars.push(<i key={i} onClick={() => onStarClicked(i)} className="question__star fa-solid fa-star"></i>);
        }
        for (let i = rating + 1; i <= 5; i++) {
            setStars.push(<i key={i} onClick={() => onStarClicked(i)} className="question__star fa-regular fa-star"></i>);
        }
        setStars(newStars);
